...
//            $this->sendNotificationToSuitableTranslators($job->id, $data, '*');// send Push for New job posting
    ...
    public function sendPushNotificationToSpecificUsers($users, $job_id, $data, $msg_text, $is_need_delay): void
    {
        ...
        $onesignalAppID = (env('APP_ENV') == 'prod') ? config('app.prodOnesignalAppID') : config(
            'app.devOnesignalAppID'
        );
        $onesignalRestAuthKey = sprintf(
            "Authorization: Basic %s",
            (env('APP_ENV') == 'prod') ? config('app.prodOnesignalApiKey') : config(
                'app.devOnesignalApiKey'
            )
        );
...
//        $jobs = TeHelper::convertJobIdsInObjs($job_ids);
    ...
    public function updateJob($id, $data, $cuser)
    {
        ...
        $current_translator = $job->translatorJobRel->first();
     ...
     * @return bool
     */
    private function getStatusChangeMethod($status, $job, $data, $changedTranslator): ?bool
    {
        switch ($status) {
            case 'timedout':
    ...
    private function changeStatus($job, $data, $changedTranslator)
    {
        ...
        if ($old_status != $data['status']) {
            $statusChanged = $this->getStatusChangeMethod($job->status, $job, $data, $changedTranslator);
    ...
    public function sendSessionStartRemindNotification($user, $job, $language, $due, $duration)
    {
        ...
        if ($this->bookingRepository->isNeedToSendPush($user->id)) {
            ...
            $this->bookingRepository->sendPushNotificationToSpecificUsers(
                ...
                $data,
                $msg_text,
                $this->bookingRepository->isNeedToDelayPush(
                    $user->id
                )
            );
    ...
    private function changeTranslator($current_translator, $data, $job)
    {
        ...
        if (!empty($data['translator_email'])) {
            ...
            if ($user) {
                $data['translator'] = $user->id;
            }
        }
        //when there is current translator
        if ($current_translator !== null || ($data['translator'] ?? 0) !== 0 || !empty($data['translator_email'])) {
            $log_data = [];
            //better code for next if
            $hasExistingTranslator = $current_translator;
            $hasNewTranslatorId = $data['translator'];
            $hasDifferentTranslatorId = $hasNewTranslatorId && $current_translator->user_id !== $data['translator'];
            $hasTranslatorEmail = $data['translator_email'] !== '';
            //simply to understand
            if ($hasExistingTranslator
                && ($hasDifferentTranslatorId || $hasTranslatorEmail)
                && $hasNewTranslatorId) {
                //good optimized way
                $new_translator = $current_translator->replicate();//clone it
                $new_translator->user_id = $data['translator'];
                $new_translator->unsetEventDispatcher();
                $new_translator->save();
            ...
            } elseif ( //when no current translator
                //using more strict '!==' here
    ...
    private function changeDue($old_due, $new_due): array
    {
        ...
        $log_data = ($dateChanged) ? ['old_due' => $old_due, 'new_due' => $new_due] : [];
    ...
    private function sendJobAcceptedEmail($user, $job): void
    {
        $email = !empty($job->user_email) ? $job->user_email : $user->email;
        $name = $user->name;
        $subject = 'Bekräftelse - tolk har accepterat er bokning (bokning # ' . $job->id . ')';
    ...
    public function acceptJobWithId($job_id, $cuser): array
    {
        ...
        if ($job->status == 'pending' && Job::insertTranslatorJobRel($cuser->id, $job_id)) {
            ...
            if ($job->status === 'assigned') {
                ...
                $response['status'] = 'fail';
                $response['message'] = 'Kunde inte acceptera bokningen för ' . $language . 'tolk ' . $job->duration . 'min ' . $job->due . '. Vänligen försök igen senare.';
            }
        }
    ...
    public function getPotentialJobs($cuser)
    {
        ...
        $languages = $cuser->userLanguages->pluck('lang_id')->all();
        $gender = $cuser_meta->gender;
        $translator_level = $cuser_meta->translator_level;
        //iterates over each item in the collection and removes any items that meet the specified condition.
        $job_ids = Job::getJobs($cuser->id, $job_type, 'pending', $languages, $gender, $translator_level)
            ->reject(
                function ($job) use ($cuser) {
                    $jobuserid = $job->user_id;
    ...
    public function getAll(Request $request, $limit = null)
    {
        ...
        if ($cuser && $cuser->user_type == env('SUPERADMIN_ROLE_ID')) {
            if (isset($requestdata['feedback']) && $requestdata['feedback'] != 'false') {
                $allJobs->where('ignore_feedback', '0')->whereHas(
                    'feedback',
                    function ($q) {
                        $q->where('rating', '<=', '3');
                    }
            ...
            if (isset($requestdata['translator_email']) && count($requestdata['translator_email'])) {
                ...
                $translatorJobRelIds = TranslatorJobRel::whereNull('cancel_at')
                    ->whereIn('user_id', $users->pluck('id'))
                    ->pluck('job_id');
                $allJobs->whereIn('id', $translatorJobRelIds);
            }
            ...
            if (isset($requestdata['salary']) && $requestdata['salary'] == 'yes') {
                $allJobs->whereDoesntHave('user.sal');
                if (isset($requestdata['salary']) && $requestdata['salary'] == 'yes') {
